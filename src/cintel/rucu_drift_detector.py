...
Paths (relative to repo root)
    ...
    INPUT FILE: data/reference_metrics_rucu.csv
    INPUT FILE: data/current_metrics_rucu.csv
    OUTPUT FILE: artifacts/drift_summary_rucu.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.rucu_drift_detector
...
REFERENCE_FILE: Final[Path] = DATA_DIR / "reference_metrics_rucu.csv"
CURRENT_FILE: Final[Path] = DATA_DIR / "current_metrics_rucu.csv"

OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "drift_summary_rucu.csv"
SUMMARY_LONG_FILE: Final[Path] = ARTIFACTS_DIR / "drift_summary_long_rucu.csv"
...
REQUESTS_DRIFT_THRESHOLD: Final[float] = 40.0
ERRORS_DRIFT_THRESHOLD: Final[float] = 2.0
LATENCY_DRIFT_THRESHOLD: Final[float] = 1000.0
# Percentage drift thresholds
REQUESTS_DRIFT_PCT_THRESHOLD: Final[float] = 30.0
ERRORS_DRIFT_PCT_THRESHOLD: Final[float] = 100.0
LATENCY_DRIFT_PCT_THRESHOLD: Final[float] = 50.0
...
def main() -> None:
    ...
    drift_df = drift_df.with_columns(
        [
            (
                (pl.col("requests_mean_difference") / pl.col("reference_avg_requests"))
                * 100
            )
            .round(2)
            .alias("requests_pct_drift"),
            ((pl.col("errors_mean_difference") / pl.col("reference_avg_errors")) * 100)
            .round(2)
            .alias("errors_pct_drift"),
            (
                (
                    pl.col("latency_mean_difference_ms")
                    / pl.col("reference_avg_latency_ms")
                )
                * 100
            )
            .round(2)
            .alias("latency_pct_drift"),
        ]
    )
    # ----------------------------------------------------
    ...
    drift_df = drift_df.with_columns(
        [
            ...
            # --------------------------------------------------------
            (pl.col("requests_pct_drift").abs() > REQUESTS_DRIFT_PCT_THRESHOLD).alias(
                "requests_pct_drift_flag"
            ),
            (pl.col("errors_pct_drift").abs() > ERRORS_DRIFT_PCT_THRESHOLD).alias(
                "errors_pct_drift_flag"
            ),
            (pl.col("latency_pct_drift").abs() > LATENCY_DRIFT_PCT_THRESHOLD).alias(
                "latency_pct_drift_flag"
            ),
        ]
    )
